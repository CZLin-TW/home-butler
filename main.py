...
- set_style：style（將使用者的風格偏好整合為精簡的 prompt 指令，30 字以內。會提供目前的自訂風格，請整合舊設定與新需求。使用者說「恢復預設」時填空字串。若使用者的描述不完整或語意不清，改用 unclear 反問確認，不要猜測填入）
...
- 使用者要求調整回覆風格、語氣、說話方式時，使用 set_style action，不要直接用新風格回覆
...
model=“claude-sonnet-4-6”,
max_tokens=500,
system=f”你是家庭專屬管家。現在是 {today} {now_time}。請根據以下資料，用溫暖簡潔的管家語氣整理成一則推播訊息。適度使用 emoji，主動補充貼心提醒（快過期的催促、今天的待辦提醒注意時間、天氣提醒帶傘或注意溫差等）。不要加開頭問候語如「早安」，直接進入內容。只回傳推播文字，不要 JSON。{style_instruction}”,
    ...
    if not result or not result.strip():
        ...
        if parsed is not None:
            ...
            if has_error:
                ...
                if raw_data:
                    # 風格注入
                    base_style = "語氣有禮簡潔，帶管家從容感，適度用 emoji。"
                    member_style = get_style_instruction(user_name, ctx)
                    style_block = f"\n{base_style}" if not member_style else f"\n預設風格：{base_style}{member_style}"
                    ...
                    if action_types & {"query_todo"}:
                        ...
                        semantic_system = f"你是家庭管家。今天是 {now_taipei().strftime('%Y-%m-%d')}。根據以下庫存數據回覆。依過期日由近到遠排序，每項一行，格式為「emoji 品名 數量單位（過期日）」。不要用 markdown 標題或分隔線。只在快過期（3天內）或已過期的品項後面補簡短提醒，其餘不加評語。" + style_block
                        semantic_max_tokens = 500
                    else:
                        semantic_system = "你是家庭管家。根據以下數據，用自然、簡潔、有溫度的語氣回覆使用者的問題。適度用 emoji。不要重複列出所有數據，挑重點回答。如果使用者問的是「冷嗎」「會下雨嗎」「濕度高嗎」這類問題，直接回答並給建議。" + style_block
