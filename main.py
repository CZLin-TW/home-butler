...
- 如果使用者在修正或補充剛才的指令（例如說「時間是下午1點」、「應該是明天」），請判斷這是在更新上一個動作，先用 delete_todo 或 delete_food 刪除舊的，再用 add_todo 或 add_food 新增正確的，不要重複新增
...
def ask_claude(user_id, user_message):
    ...
    prompt = SYSTEM_PROMPT.format(today=today, now_time=now_time, family_info=family_info, food_info=food_info)
