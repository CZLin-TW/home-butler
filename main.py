...
- control_ir：控制 DIY 紅外線設備（需要 device_name, button）
  ...
  button 請填按鈕名稱，必須與設備支援的按鈕完全一致
  可用的設備與按鈕：{ir_device_info}
  範例情境：
  - 「開電風扇」→ device_name="電風扇", button="電源"
  - 「風扇風速大一點」→ device_name="電風扇", button="風速+"
  - 「關電風扇」→ device_name="電風扇", button="電源"
...
{{"actions": [{{"action": "control_ir", "device_name": "電風扇", "button": "電源"}}], "reply": "好的，電風扇已開啟。"}}
...
def handle_control_ir(data):
    ...
    result = switchbot_api.ir_custom_button(device_id, button)
    if result.get("success"):
