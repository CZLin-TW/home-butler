...
請只回傳 JSON 物件，不要有其他文字。格式如下：
{
  "actions": [...],
  "reply": "用管家語氣寫給使用者看的回覆，自然、簡潔、有禮"
}
...
{"actions": [{"action": "query_food"}], "reply": "（由程式填入庫存清單）"}
    ...
    try:
        ...
        for data in actions:
            ...
            if action == "add_food":
                ...
                pass  # unclear 的回覆直接用 claude_reply

        # query 類的結果要顯示，其他用 claude_reply
