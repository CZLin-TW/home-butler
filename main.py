...
- modify_food：修改食品資料（需要 name，選填：new_name, quantity, expiry，只填要修改的欄位）
  quantity 歸零時程式會自動標記已消耗
...
- modify_todo：修改待辦事項（需要 item，選填：new_item, date, time, person, type，只填要修改的欄位）
- delete_todo：標記待辦事項為已完成（需要 item）
- query_todo：查詢待辦事項（選填：filter，值為 "today" 時只顯示今天的待辦）
...
- 如果使用者詢問特定品項是否存在，請直接根據目前食品庫存回答，不需要使用 query_food
- query_food 只在使用者要求查看完整庫存清單時使用
- 使用者問「今天有什麼事」→ query_todo 帶 filter: "today"
- 使用者問「我有哪些待辦」→ query_todo 不帶 filter
...
{{"actions": [{{"action": "query_todo"}}], "reply": "以下是您目前所有的待辦事項：\n（由程式填入）"}}
{{"actions": [{{"action": "query_todo", "filter": "today"}}], "reply": "今天需要處理的事項如下：\n（由程式填入）"}}
...
def handle_modify(data):
    ...
    archive = get_sheet("食品封存")
    records = sheet.get_all_records()
    new_quantity = int(data.get("quantity", 0)) if data.get("quantity") is not None else None
    for i, row in enumerate(records):
        if row.get("品名") == data.get("name") and row.get("狀態") == "有效":
            ...
            if data.get("new_name"):
                sheet.update_cell(i + 2, 1, data.get("new_name"))
            if data.get("expiry"):
    ...
    for i, row in enumerate(records):
        if row.get("事項") == data.get("item") and row.get("狀態") == "待辦":
            if data.get("new_item"):
                sheet.update_cell(i + 2, 1, data.get("new_item"))
            if data.get("date"):
...
def handle_query_todo(user_name, filter=None):
    sheet = get_sheet("待辦事項")
    records = sheet.get_all_records()
    valid = [r for r in records if r.get("狀態") == "待辦"]
    if not valid:
    ...
    for r in valid:
        ...
        if todo_type == "私人" and person != user_name:
            continue
        if filter == "today" and r.get("日期") != today:
            continue
        time_part = f" {r['時間']}" if r.get("時間") else ""
    ...
    try:
        ...
        for data in actions:
            ...
            if action == "add_food":
                ...
                results.append(handle_query_todo(user_name, filter=data.get("filter")))
            elif action == "unclear":
                pass
        ...
        if has_query:
            query_result = "\n".join(results)
            reply = claude_reply.replace("（由程式填入）", query_result) if "（由程式填入）" in claude_reply else query_result
        else:
