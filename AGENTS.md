...
Dashboard 可複選現有照明區域，API 使用 `light_area_ids`；舊 `light_area_id`／`light_area` 仍相容。
沿用「燈光區域ID」欄：單一 ID 保持純字串，多個 ID 存 JSON 陣列，不新增 Sheet 欄位。
週期模板與生成實例保留全部 ID。派送前展開成各區域，HA／legacy PC 共用原提醒格式；
維持固定 breathe、每區域每分鐘最多一次，完成／停用後不再提醒。不改 HA 區域歸屬或通知效果。
