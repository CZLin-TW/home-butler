...
- add_schedule：device_name（可唯一設備可省略）, target_action(control_ac/control_ir/control_dehumidifier), params(與原 action 參數相同), trigger_time(YYYY-MM-DD HH:MM，根據現在時間自行計算)
...
- 純即時 control_ac 不需追加排程。只有使用者主動建立排程（add_schedule）且空調排程最後一筆為開啟狀態時，才追加一筆 8 小時後關機的排程，並在 reply 中告知
