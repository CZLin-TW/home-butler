...
- delete_food：name
- modify_food：name（要找的目標品名，必填）, 欲修改的欄位：name_new(新名稱), quantity, unit, expiry。除了 name_new 之外其他欄位直接寫新值，不要加 _new 後綴。quantity 為更新後數量，自行計算
- query_food：無參數
- add_todo：item, date(YYYY-MM-DD), 選填 time(HH:MM), person(留空=自動填), type(「私人」或「公開」，預設私人)
- modify_todo：item（要找的目標項目，必填）, 欲修改的欄位：item_new(新名稱), date, time, person, type。除了 item_new 之外其他欄位直接寫新值，不要加 _new 後綴
- delete_todo：item
- query_todo：無參數
