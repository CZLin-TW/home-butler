...
v1.56.0 在 Dashboard 空調卡恢復「自動關機」，沿用 Sheet 的「自動關機小時數」：0／空白停用，介面可設 1–168 小時。
HA 空調由 HB 觀察開機並計時，到期透過 HA 關閉；Apple Home／HA 開機也能涵蓋，不建立 HA 自動化。
調溫不重置，手動關機排程優先；HA 斷線不視為關機，未知結果不重送。防黴與回饋仍維持停用。
[計時語意與限制](docs/ac-auto-off.md)。不需更新 HA 整合。

## 手動排程更新

v1.55.0 恢復 Dashboard 的 HA 空調一次性手動排程：指定日期時間後，由 HB 經 HA 下達控制。
可新增、編輯與取消，支援開關、整數溫度、模式、風速；排程卡與表單同步統一視覺。
原本停用的 HA 空調防黴、回饋與自動關機不恢復；舊排程不會因更新而自動重新生效。
...
Dashboard 不會編輯 HA 自動化；新建的手動排程由 HB 經 HA 執行，舊防黴與回饋補償不對已遷移空調執行；v1.56.0 自動關機依既有時數欄位重新建立 HA 計時。
