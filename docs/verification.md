...
2026-09-13 HA 第一階段（候選 v1.45.0，尚未發布）：本機 186 項後端測試通過，
包含真實 FastAPI／WebSocket 的獨立金鑰、格式驗證、false／0／未知、斷線、
重連與舊連線隔離。Python 編譯與安裝腳本語法檢查通過。
另以假傳輸驗證斷線重連後重新讀取最新快照、不補播舊值，測試通過。
新增 Linux／Python 3.14／HA Core 2026.9.2 的 registry、設定流程、生命週期 CI，
尚未執行：本次 main 提交／推送被自動審核拒絕，等待使用者重新明確確認發布。
不能把這些本機結果寫成 HA 整合已載入或 FP2 已回傳成功；Render 專用 key、
