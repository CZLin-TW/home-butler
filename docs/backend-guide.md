...
都由 HA 提供控制或資料；以下直接雲端、PC Hue、舊空調補償／排程內容僅適用未切換的設備。
...
上述工作只有在後端程序運行時才會執行。既有 UptimeRobot 監控用途見「十三、常駐與健康檢查」，變更主機方案時需一併考慮。HA Hue 提醒另有每 60 秒的 `lighting-reminders` 工作；HA 管理空調不執行舊補償與 HB 排程。
...
| /api/schedules | POST | 新增 HB 排程；不接受已遷移 HA 空調的舊排程，請在 HA 設定 |
