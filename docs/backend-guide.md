...
FP2 經 HomeKit 接入 HA，再由 HA 同步選定觀測回 HomeButler；不需要 Aqara 開發者帳號。
安裝見 [HA 整合](../homeassistant/README.md)，走這條路的原因見 [動機與限制](why-home-assistant.md)。

舊的 `aqara_api.py` 雲端探索工具已移除：Aqara 開發者 API 需要實名制申請，使用者決定不申請，
那份程式碼因此從未對真機執行過。需要 FP2 資料請走 HA，不要重新實作雲端路徑。
