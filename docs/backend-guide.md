...
上述工作只有在後端程序運行時才會執行。既有 UptimeRobot 監控用途見「十三、常駐與健康檢查」，變更主機方案時需一併考慮。HA Hue 提醒另有每 60 秒的 `lighting-reminders` 工作；HA 管理空調不執行舊補償與舊來源自動排程，新建手動排程與 v1.56.0 自動關機經 HA 執行。
...
未遷移 HA 的冷氣控制仍由原 handler 維護既有防黴收尾與自動關機排程；HA 管理空調不執行舊防黴；v1.56.0 的自動關機由 HB 每分鐘觀察 HA 狀態另行計時。這是設備控制的必要副作用，不代表此金鑰可直接管理排程。除濕機自動模式鎖定時，仍需由完整 Dashboard 關閉自動模式才能手動控制，家電捷徑不能繞過鎖定。
