...
| `ac.auto_schedule` | 未遷移空調的排程維護；只有走到該分支才出現，不改變排程原有規則 |
...
`tests/test_sheets_reuse.py` 驗證連線並行初始化、跨時間重用、失敗失效與重建、最新欄列／空白列定位、缺表／重複欄名／ID 拒寫、未知寫入不重試，以及按 Device ID 定位快取列。全部使用假 SDK／Sheets，不等同正式憑證過期或服務重啟的實測。
