...
6. 檢查 Dashboard 顯示「由 HA 管理」：整數調溫、HA 模式／風速、HA 斷線未知；回饋與舊排程介面退出。
...
已遷移空調的 HB 補償、防黴、自動關機及所有 HB 空調排程停止執行（到期的舊排程標記已取消）；
新排程請改在 HA 設定。舊 Sheet 設定與歷史保留但不作目前狀態來源，不能以空值回退舊讀值。
