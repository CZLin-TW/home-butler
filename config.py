...
# Separate capability: accepted ONLY by POST /api/assistant/devices. Empty disables it.
...
# Dedicated HA link authentication; never accepted by owner API verifiers.
HOME_ASSISTANT_API_KEY = os.environ.get("HOME_ASSISTANT_API_KEY", "")
# Explicit AC ownership transfer. Selected names never fall back to direct IR.
HOME_ASSISTANT_AC_NAMES = os.environ.get("HOME_ASSISTANT_AC_NAMES", "[]")
...
# 但無名字/無風格/對話記憶獨立一份。空白 ID 不會限制權限；家電專用請改走
# /api/assistant/devices + DEVICE_VOICE_API_KEY，該入口不使用 SIRI_USER_ID。
